class AdocGtProductPage {
  ...
  async selectAvailableVariants() {
    ...
    // Select Size (Talla)
    const sizeSwatches = this.page.locator('.swatch-element.talla label, .size-swatch, .variant-input[data-option-name="Talla"] label, [data-option="option2"] label, a.swatch-element');
    
    // We can just try to click sizes directly using class matching since text contains whitespace
    const genericSizeSwatches = this.page.locator('ul.options-talla a, ul.options-size a, .clickyboxes a');
...
module.exports = { AdocGtProductPage };
